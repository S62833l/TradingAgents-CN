...
class SimpleAnalysisService:
    ...
    def __init__(self):
        ...
        # 从环境变量读取并发数，默认10（已解除开源版3的限制）
        import concurrent.futures
        import os as _os
        _workers = int(_os.getenv("ANALYSIS_CONCURRENT_WORKERS", "10"))
        self._thread_pool = concurrent.futures.ThreadPoolExecutor(max_workers=_workers)
    ...
    def _resolve_stock_name(self, code: Optional[str]) -> str:
        ...
        # 兜底：直接从 MongoDB stock_basic_info 查
        if not name:
            try:
                ...
                if db is not None:
                    ...
                    if doc and doc.get("name"):
                        name = doc["name"]
                        logger.info(f"✅ MongoDB兜底获取名称: {code} -> {name}")
            except Exception:
                pass
        if not name:
            name = f"股票{code}"
